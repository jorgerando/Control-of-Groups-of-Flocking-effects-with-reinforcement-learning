...
if __name__ == "__main__":
    ...
    dim = 10
    env = followAgents(len=10,n=5,block=True ,n_actions=5,random_=True,move=True,max_steps=500)
    #env = gym.make("LunarLander-v2")
    writer = SummaryWriter(comment="-FollowAgents-"+str(os.getpid()))
    ...
    for step_idx, exp in enumerate(exp_source):
        ...
        if new_rewards:
            ...
            if step_idx > 1_000_000 :
                print("Solved in %d steps and %d episodes!" % (step_idx, done_episodes))
                torch.save(net.state_dict(),'./Falso')
                break
    ...
    env = followAgents(len=1000 ,n=5,block=True,n_actions=5,random_=False,move=True)
    state = env.reset()
