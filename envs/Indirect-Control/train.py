...
if __name__ == "__main__":
    ...
    dim = 10
    env = followMe(len=20,n=1,r_=2,n_actions=4)
    writer = SummaryWriter(comment=str(os.getpid()))
    ...
    for step_idx, exp in enumerate(exp_source):
        ...
        if new_rewards:
            ...
            if mean_rewards > 1_000_000 : #20 :
                print("Solved in %d steps and %d episodes!" % (step_idx, done_episodes))
                torch.save(net.state_dict(),'./00')
                break
    ...
    env = followMe(len=500,n=1,r_=50,n_actions=4,max_steps=1000)
    state = env.reset()
