...
class My(Scene):
    ...
    def construct(self):
        ...
        env = defendCircle(len=1000,r_=130,vmax=50,fmax=0.85,random_=True,max_steps=2500,change_point=True)
        state = env.reset()
        ...
        for i in range(10000) :
                ...
                point = Circle(radius=2,color=WHITE,stroke_width=250)
                point.set_fill(WHITE, opacity=1)
                point.move_to( (env.point[0],env.point[1],0) )

                radio = Circle(radius=env.r/2,color=WHITE,stroke_width=250)
                radio.move_to( (env.point[0],env.point[1],0) )

                huir = Circle(radius=env.r,color=PURE_RED)
                huir.set_fill(PURE_RED, opacity=0.5)
                huir.move_to( (env.agent.p[0],env.agent.p[1],0) )

                self.add(point,radio,huir)

                diffc = np.linalg.norm(np.array(env.point) - env.target.getP())

                if diffc < env.r/2 :

                    in_ = Circle(radius=env.r/2,color=PURE_GREEN,stroke_width=250)
                    in_.move_to( (env.point[0],env.point[1],0) )
                    self.add(in_)

                    point = Circle(radius=2,color=PURE_GREEN,stroke_width=250)
                    point.set_fill(PURE_GREEN, opacity=1)
                    point.move_to( (env.point[0],env.point[1],0) )
                    self.add(point)


                env.target.maninDraw(self,BLUE)
